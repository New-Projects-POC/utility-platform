...
// ── Constants ─────────────────────────────────────────────────────────────────
const LP_API           = '/api/mdm/meter-data/getCurrUiBillingData/query';
const PAGE_SIZE        = 10;
...
export function CurrentBillingData() {
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      if (parsed.content.length === 0 && parsed.headers.length === 0) {
        console.warn('[CurrentBillingData] parseResponse returned empty — raw res:', res);
      }
        ...
        <div className="card">
          ...
          {loading && (
            <div style={{ padding: '4rem', textAlign: 'center', color: 'var(--text3)' }}>
              ...
              <div style={{ fontSize: 12 }}>Fetching current billing data…</div>
            </div>
          )}
...
export default CurrentBillingData;