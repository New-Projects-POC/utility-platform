...
// ── Constants ─────────────────────────────────────────────────────────────────
const LP_API           = '/api/mdm/meter-data/getNamePlate/query';
const PAGE_SIZE        = 10;
...
// AFTER — just replace T with space, seconds pass through as-is
startDate: startDate.replace('T', ' '),   // "2026-06-25 00:00:00"
endDate:   endDate.replace('T', ' '),     // "2026-06-25 14:32:47"
    cursorTime,
    cursorDevice,
    page:      0,
    totalRows: 0,
    totalPages: 0,
  }), [levelName, levelValue, startDate, endDate, tenantCode]);
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      if (parsed.content.length === 0 && parsed.headers.length === 0) {
        console.warn('[LoadProfile] parseResponse returned empty — raw res:', res);
      }
  ...
  // ── AUTO-BIND: headers and values both come purely from the API response ───
        ...
        <div className="card">
          ...
          {loading && (
            <div style={{ padding: '4rem', textAlign: 'center', color: 'var(--text3)' }}>
              ...
              <div style={{ fontSize: 12 }}>Fetching name plate data…</div>
            </div>
          )}
            ...
            <>
              <div style={{ overflowX: 'auto' }}>
                <table className="data-table">
                  <thead>
                    <tr>
                      {/* ── AUTO-BIND: render exactly the headers the API returned ── */}
                      {colHeaders.map((h, i) => (
                  ...
                  <tbody>
                    {uiRows.map((row, ri) => (
                      <tr key={ri}>
                        {/* ── AUTO-BIND: render each cell by its positional index ── */}
                        {colHeaders.map((_, ci) => (
                          <td key={ci} style={{
